...
class HrPlanilla(models.Model):
    ...
    def action_generate_scv_csv(self):
        self.ensure_one()
        content = ""
        for line in self.planilla_lines:
            # Cuenta (solo números)
        ...
        # Guardar en el campo doc_csv existente en hr.planilla (de treming_sv_payroll)
        self.write({'doc_csv': content_b64})
        
        # Retornar acción para descargar el archivo
        return {
            'type': 'ir.actions.act_url',
            'url': f'/web/content/?model=hr.planilla&id={self.id}&field=doc_csv&download=true&filename=planilla_scv.csv',
            'target': 'self',
        }
